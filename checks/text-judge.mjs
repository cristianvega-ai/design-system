...
// TypeSafe's System One model with the spec's list of approved words, and asks one yes-or-no
...
export const DEFAULT_FILES = [
  ...
  'CHANGELOG.md',
  'fonts/README.md',
  'docs/superpowers/specs/2026-09-15-design-system-completion-design.md',
];
export const SPEC_FILE = 'docs/superpowers/specs/2026-09-15-design-system-completion-design.md';
...
export function markdownRuns(text) {
  ...
  lines.forEach((raw, index) => {
    const line = index + 1;
    if (/^\s*```/.test(raw)) { inFence = !inFence; endParagraph(); return; }
    if (inFence) return;
...
// Descriptive runs of the sheet, through the text report's walk.
export function sheetRuns(html) {
  return descriptiveRuns(html).map((run) => ({ line: lineOf(html, run.index), text: run.text.replace(/\s+/g, ' ').trim() }));
}
...
// The approved words from the spec's Terms section: each bullet "- Word: meaning".
export function glossaryFrom(spec) {
  const section = /^## 2\. Terms\s*$([\s\S]*?)(?=^## )/m.exec(spec);
  if (!section) return [];
  const glossary = [];
...
export function unitsFrom(files, read = (file) => readFileSync(file, 'utf8')) {
  ...
  for (const file of files) {
    ...
    const runs = file.endsWith('.html') ? sheetRuns(text) : markdownRuns(text);
...
export async function judge(options, deps = {}) {
  ...
  const glossary = glossaryFrom(read(SPEC_FILE));
