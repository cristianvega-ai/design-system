...
// A report on the descriptive text of the sheet and the products document. It is a report, not a
// check: it always exits 0, and run.sh does not run it. Run it by hand: node checks/text-report.mjs
//
...
// Ruling R22 adds the names the products document needs: a database, a view class, a search
...
export function textReport(html) {
  const sentences = [];
  for (const run of descriptiveRuns(html)) {
    const line = lineOf(html, run.index);
    for (const piece of run.text.replace(/\s+/g, ' ').trim().split(/(?<=[.!?])\s+/)) {
      if (piece) sentences.push({ line, text: piece, words: countWords(piece) });
    }
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  for (const file of files.length ? files : FILES) {
    const { longSentences, acronyms } = textReport(readFileSync(file, 'utf8'));
