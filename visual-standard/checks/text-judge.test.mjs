...
import { splitSentences, glossaryFrom, unitsFrom, buildBatches, collectFlags, formatReport, askBatch } from './text-judge.mjs';
...
test('glossaryFrom reads every term bullet of the glossary', () => {
  const text = [
    '# Glossary',
    '',
  ...
  assert.deepEqual(glossaryFrom(text), [
...
test('unitsFrom skips the front matter of a Markdown file', () => {
  const read = () => ['---', 'title: A record', 'owner: ABC', '---', '', 'One sentence.'].join('\n');
  assert.deepEqual(unitsFrom(['a.md'], read), [{ file: 'a.md', line: 6, sentences: ['One sentence.'] }]);
});
