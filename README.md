...
This repository holds the Cristian Vega design system. The visual standard lives in `visual-standard/`. The technical standard lives in `technical-standard/`. The sheet, `cristian-vega-design-system.html`, holds every token, component, and rule. `tokens.json` is the token file, the source of every token. The build reads the token file and writes `dist/tokens.css` and four regions of the sheet. The checks confirm the sheet, the token file, and the build agree. `AGENTS.md` holds the instructions for coding agents. `docs/` holds the records and the glossary.
...
Run every command in this file from `visual-standard/`. Run `npm install` once to fetch the pinned tools. Run `npm run build` to write `dist/tokens.css` and the sheet's four generated regions. The build needs Node 24 or newer.
...
Run `node checks/text-judge.mjs` for the text judge. It sends every descriptive sentence of the sheet and the documents to TypeSafe's System One model. It asks four questions per sentence. Is the voice passive? Does the sentence hold more than one topic? Does it use another word for an approved term? Does the detail come before the instruction? It prints the sentences at or above the threshold, worst first. It is a report for a person to read, not a check. It needs a key in `TYPESAFE_API_KEY` or in `~/.config/typesafe/api_key`, and a network connection. Options: `--threshold=0.5`, `--limit=40`, `--files=../README.md,../CHANGELOG.md`.
